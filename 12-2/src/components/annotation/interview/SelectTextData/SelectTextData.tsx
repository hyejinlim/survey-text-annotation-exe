...
import { fetchInterviewToolList } from '~/api/fetches/fetchInterview';
...
function SelectTextData() {
  ...
  const { setInterviewId } = useContext(InterviewTextAnnotationContext);
  ...
  const init = (params?: any) => {
    setLoading(true);
    (async () => {
      const { response } = await fetchInterviewToolList({
        paging: true,
        pagdSize: PAGE_SIZE,
        ...params,
      });
  ...
  const handleClick = async () => {
    setMoreLoading(true);
    const { response, error } = await fetchInterviewToolList({
      paging: true,
      pagdSize: PAGE_SIZE,
      lastId,
      ...searchQuery,
  ...
  const handleSelectDocument = (id: any) => {
    navigate(`/interview/labeling/${id}`);
  };

  /** useEffect */
  useEffect(init, []);
  useEffect(() => {
    if (interviewId && +interviewId > 0) setInterviewId(interviewId);
  }, [interviewId]);
          ...
          <div css={styles.container}>
            <div css={styles.dataContainer}>
              {documents.map((item: any) => {
                const { interviewId, interviewTitle } = item;
                return (
                  <div
                    key={interviewId}
                    role="button"
                    onClick={() => handleSelectDocument(interviewId)}
                  >
