...
import { fetchSurveyDelete, fetchSurveyList } from '~/api/fetches/fetchSurvey';
...
function SurveyDataList({ initData }: Props) {
  ...
  const {
    ...
    queryKey: ['surveyData', selectedValues, page, limit],
    queryFn: () => fetchSurveyList({ ...selectedValues }),
  });

  const { downloadName, downloadUrl } = surveyData?.response || {};

  const deleteMutation = useMutation(fetchSurveyDelete);
  ...
  const isLoading = authorityLoading || surveyDataLoading;

  return (
    <TableApiPaginationContainer
      columns={columns(selectedValues)} // TODO: 컬럼 메모이제이션
      data={surveyData?.response.payload || []}
      ...
      totalRow={surveyData?.response.totalRow}
      curPage={page}
      totalPage={surveyData?.response.totalPage}
...
export default memo(SurveyDataList);
