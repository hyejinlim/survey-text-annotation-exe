...
function DashboardInterview() {
  ...
  const { data: statisticsCard, isLoading: statisticsCardLoading } = useQuery({
    queryKey: ['statistics'],
    queryFn: () => fetchInterviewStatisticsCard(),
  });

  // 개별 통계 상세
  const { data: statisticsCardDetail } = useQuery({
    queryKey: ['statisticsDetail', { type: statisticsId }],
    queryFn: () => fetchInterviewStatisticsCardDetail({ type: statisticsId }),
  });
  ...
  return (
    <div className="page-content">
      <Container fluid>
        <Breadcrumb title="대시보드" breadcrumbItem="12-2 대시보드" />
        <StatisticsCard
...
export default memo(DashboardInterview);
