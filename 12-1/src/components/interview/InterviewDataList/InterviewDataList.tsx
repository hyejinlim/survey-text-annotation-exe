...
import IntervewDataSelectFilter from '../IntervewDataSelectFilter';

type Props = {
  initData: any;
};

function InterviewDataList({ initData }: Props) {
  const { listType, ...rest } = initData;
  ...
  const {
    ...
    queryKey: ['interviewData', selectedValues, page, limit],
    queryFn: () => fetchInterviewList({ ...selectedValues }),
  });

  const { downloadName, downloadUrl } = interviewData?.response || {};

  const deleteMutation = useMutation(fetchInterviewDelete);
  ...
  const isLoading = authorityLoading || interviewDataLoading;

  return (
    <TableApiPaginationContainer
      columns={columns(selectedValues)} // TODO: 컬럼 메모이제이션
      data={interviewData?.response.payload || []}
      ...
      totalRow={interviewData?.response.totalRow}
      curPage={page}
      totalPage={interviewData?.response.totalPage}
...
export default memo(InterviewDataList);
