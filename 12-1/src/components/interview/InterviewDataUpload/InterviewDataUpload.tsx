...
import {
  fetchInterviewFileInsert,
  fetchInterviewFileUpload,
} from '~/api/fetches/fetchInterview';
import { MENU0401 } from '~/constants/menu';
 ...
 * insert: 최종 업로드
 */
function InterviewDataUpload() {
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [selectedFiles, setSelectedFiles] = useState<any>([]);
  const [uploadLoading, setUploadLoading] = useState<boolean>(false);

  const { data: authority } = useCheckAuthorityQuery(MENU0401);

  const surveyFileUploadMutation = useMutation(fetchInterviewFileUpload);
  const surveyFileInsertMutation = useMutation(fetchInterviewFileInsert);
...
export default memo(InterviewDataUpload);
